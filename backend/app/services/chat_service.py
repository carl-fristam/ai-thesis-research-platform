...
class ChatService:
    ...
    async def process_query(self, query: ChatQuery, user_id: str):
        ...
        system_message = [
            ...
            {
                "type": "text", 
                "text": "You are a research assistant. You have access to a Library Overview (all titles) and Relevant Document Excerpts (content). Use the Library Overview for counting/grouping questions, and Relevant Excerpts for specific content questions. Format with Markdown. IMPORTANT: When mentioning a saved source, you MUST link it using markdown format: [Title](URL) using the URL provided in the Library Overview."
            }
        ]
